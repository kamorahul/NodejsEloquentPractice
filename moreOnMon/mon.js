...
//console.log("ha");
return 1
  }else if(a.constructor==Object && !b.localeCompare("Object"))
  {
return 1
  } 
  else if(b.match(/RegExp./g))
  {
    var rep=b.replace('RegExp-*/','g');       
...
module.exports={
  ty:function typeC(obj,obj1){
   ...
   {//console.log(p[keys[i]].constructor);
    if(p[keys[i]].constructor==Object)
      {
        if(n[keys[i]].constructor==String){
      f.foo(n[keys[i]],p[keys[i]]);}
      else if(p[keys[i]].constructor==Object && n[keys[i]].constructor==Number){
        //console.log("hi");
        f.Num(n[keys[i]],p[keys[i]]);
      }
    }else
